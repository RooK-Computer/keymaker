...
func (NoopRunner) Run(ctx context.Context, cmd string, args ...string) (string, string, error) {
	return "", "", nil
}
...
func (sr ShellRunner) Run(ctx context.Context, cmd string, args ...string) (string, string, error) {
	...
	command := exec.CommandContext(ctx, "sudo", fullArgs...)
	...
	if err != nil {
		...
		if exitErr, ok := err.(*exec.ExitError); ok {
			if sr.Logger != nil {
				sr.Logger.Errorf("system", "cmd failed: sudo %s %v, exit=%d, stderr=%s", cmd, args, exitErr.ExitCode(), truncate(errBuf.String(), 256))
			}
			return outBuf.String(), errBuf.String(), fmt.Errorf("exit %d: %w", exitErr.ExitCode(), err)
		}
		if sr.Logger != nil {
			sr.Logger.Errorf("system", "cmd failed: sudo %s %v, err=%v", cmd, args, err)
		}
		return outBuf.String(), errBuf.String(), err
	}
	if sr.Logger != nil {
		sr.Logger.Infof("system", "cmd ok: sudo %s %v, stdout=%s", cmd, args, truncate(outBuf.String(), 256))
	}
