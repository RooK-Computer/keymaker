...
We did check the load of the system while the main screen is visible and noticed that way more threads of keymaker were running and actively using the cpu than we anticipated. We thought that 2 processes should show up. instead it was constantly between 4 and 6. the load of the system reached 3. We have to re-evaluate the implementation of the main screen. something is off.
...
Now hats implemented, its not working on the main screen. F4 is not doing anything.
...
While we already tried fixing that, there is no change on the main screen. it still shows no data for a few seconds, then shows the info and almost immediately afterwards shows the qr codes.
